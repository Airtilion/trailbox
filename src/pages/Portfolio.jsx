...
const Portfolio = () => {
  ...
  return (
    <section className='w-[1248px] mx-auto my-[64px] max-2xl:w-[932px] max-lg:w-[616px] max-sm:w-[90%]'>
      <h2 className='text-[30px] font-semibold w-[450px] max-lg:w-full max-md:text-[20px] max-sm:w-[90%] max-sm:mx-auto'>
        Realizacje - Galeria zdjęć
      </h2>
      <p className='text-[16px] font-light flex-1 max-lg:text-[15px] max-md:text-[14px] max-sm:w-[90%] max-sm:mx-auto'>
      ...
      <article className='grid grid-cols-4 gap-[16px] my-[64px] max-2xl:grid-cols-3 max-lg:grid-cols-2 max-sm:flex max-sm:flex-col max-sm:items-center'>
        {visibleImages.map((src, index) => {
          ...
          const classes = isSpecial
            ? 'col-span-2 row-span-2 w-[620px] h-[420px] rounded-[5px] object-cover max-sm:w-[300px] max-sm:h-[200px]'
            : 'w-[300px] h-[200px] rounded-[5px] object-cover';
